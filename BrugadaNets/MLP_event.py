...
def plot_column_hist(dataframe, dataframe1, dataframe0, prefix=''):
    ...
    for ax, tit, feat in zip(axs.flatten(), titles, dataframe.columns[0:21]):
...
def model_metrics(classifier, estimator, train_features, train_labels, test_features, test_labels, pred_labels_test, pred_labels_train, prefix='', suffix=''):
    ...
    print('*************** Model Summary ***************')
    print('No. of classes: ', len(classifier.classes_))
    print('No. of layers: ', classifier.n_layers_)
    print('No. of features: ', classifier.n_features_in_)
    print('Current loss: ', classifier.loss_)
    # print('Best loss: ', classifier.best_loss_)
...
correlation_blocks(df, 'correlation_blocks.pdf', 'MLP_')
correlation_blocks(event_df, 'event_correlation_blocks.pdf', 'MLP_')
correlation_blocks(nonevent_df, 'nonevent_correlation_blocks.pdf', 'MLP_')
...
plot_column_hist(df, event_df, nonevent_df, 'MLP_')
...
# mdl_1 = MLPClassifier(hidden_layer_sizes=(50), activation='relu', solver='lbfgs', alpha=1.e-03, max_iter=1, warm_start=True, random_state=42)
# # mdl_1 = MLPClassifier(hidden_layer_sizes=(50), activation='relu', solver='sgd', alpha=1.e-03, learning_rate='adaptive', learning_rate_init=1.e-03, early_stopping=True, max_iter=1, warm_start=True, random_state=42)

# for i in range(20):
#     clf_1 = mdl_1.fit(X_train_sc, y_train)
mdl_1 = MLPClassifier(hidden_layer_sizes=(50), activation='relu', solver='lbfgs', alpha=1.e-03, random_state=42)
clf_1 = mdl_1.fit(X_train_sc,y_train)

y_pred_te = mdl_1.predict(X_test_sc)
y_pred_tr = mdl_1.predict(X_train_sc)

print(mdl_1)

model_metrics(clf_1, mdl_1, X_train_sc, y_train, X_test_sc, y_test, y_pred_te, y_pred_tr, prefix='MLP_')
...
# model_metrics(opt_clf, opt_xgb, X_train, y_train, X_test, y_test, y_pred_te_opt, y_pred_tr_opt, prefix='MLP_', suffix='_opt')